    ...
    <aside className={`hidden lg:flex flex-col ${isCollapsed ? 'w-16' : 'w-[260px]'} h-screen fixed left-0 top-0 z-40 transition-all duration-300`}
      ...
      <div className={`${isCollapsed ? 'px-2' : 'px-3'} pb-2`}>
        <Link
          ...
          className={`flex items-center ${isCollapsed ? 'justify-center w-12 h-12 mx-auto' : 'gap-3 px-4 py-3'} rounded-xl text-sm font-medium transition-all duration-200
            ${pathname.startsWith('/settings') ? 'nav-item-active shadow-sm' : 'hover:opacity-80'}`}
          style={{ color: pathname.startsWith('/settings') ? 'var(--accent)' : 'var(--text-secondary)' }}
        >
          <svg className="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={pathname.startsWith('/settings') ? 2.5 : 1.5}>
      ...
      {/* Bottom info & Toggle */}
      <div className={`px-4 py-4 flex flex-col ${isCollapsed ? 'items-center' : ''}`} style={{ borderTop: '1px solid var(--border-secondary)' }}>
        {!isCollapsed && (
          <div className="flex items-center gap-2 text-xs px-2 mb-4" style={{ color: 'var(--text-tertiary)' }}>
        ...
        <button
          ...
          className={`flex items-center justify-center ${isCollapsed ? 'w-10 h-10' : 'w-full py-2.5 gap-2'} rounded-xl text-xs font-bold transition-colors hover:bg-black/5 dark:hover:bg-white/5 haptic-tap`}
          style={{ color: 'var(--text-secondary)' }}
          title={isCollapsed ? 'Genişlet' : 'Daralt'}
        >
          <svg className={`w-5 h-5 transition-transform duration-300 ${isCollapsed ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
